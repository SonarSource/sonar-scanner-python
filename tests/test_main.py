...
class TestMain(unittest.TestCase):
    ...
    def test_short_cli_args(self):
        ...
        expected_internal = Internal()
        expected_scanner = Scanner(internal=expected_internal)
        expected_sonar = Sonar(scanner=expected_scanner,
                               token='myToken', verbose=True)
        expected_configuration = Configuration(sonar=expected_sonar)
        self.assertEqual(configuration, expected_configuration)
    ...
    @patch('sys.argv', ['myscript.py',
                        ...
                        '--sonar-region', 'us',
                        '--sonar-user-home', 'mySonarUserHome',
                        '--sonar-scanner-cloud-url', 'mySonarScannerCloudUrl',
                        '--sonar-scanner-api-url', 'mySonarScannerApiUrl',
                        '--sonar-scanner-os', 'windows',
                        '--sonar-scanner-arch', 'x64',
                        '--skip-jre-provisioning',
                        '--sonar-scanner-java-exe-path', 'mySonarScannerJavaExePath',
                        '--sonar-scanner-java-opts', 'mySonarScannerJavaOpts',
                        '--sonar-scanner-internal-dump-to-file', 'mySonarScannerInternalDumpToFile',
                        '--sonar-scanner-internal-sq-version', 'mySonarScannerInternalSqVersion',
                        '--sonar-scanner-connect-timeout', '42',
                        '--sonar-scanner-socket-timeout', '43',
                        '--sonar-scanner-response-timeout', '44',
                        '--sonar-scanner-truststore-path', 'mySonarScannerTruststorePath',
                        '--sonar-scanner-truststore-password', 'mySonarScannerTruststorePassword',
                        '--sonar-scanner-keystore-path', 'mySonarScannerKeystorePath',
                        '--sonar-scanner-keystore-password', 'mySonarScannerKeystorePassword',
                        '--sonar-scanner-proxy-host', 'mySonarScannerProxyHost',
                        '--sonar-scanner-proxy-port', '45',
                        '--sonar-scanner-proxy-user', 'mySonarScannerProxyUser',
                        '--sonar-scanner-proxy-password', 'mySonarScannerProxyPassword',
                        '--sonar-project-base-dir', 'mySonarProjectBaseDir'
                        ])
    def test_all_cli_args(self):
        configuration = initialize_configuration()
        expected_internal = Internal(dump_to_file='mySonarScannerInternalDumpToFile', sq_version='mySonarScannerInternalSqVersion')
