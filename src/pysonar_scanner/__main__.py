...
def parse_cli_args():
    ...
    parser.add_argument(
        "--sonar-region", type=str, default="", choices=["us"], help="The region to contact, only for SonarQube Cloud"
    )
    ...
    parser.add_argument(
        "--sonar-scanner-os", type=str, choices=["windows", "linux", "macos", "alpine"], help="OS running the scanner"
    )
    parser.add_argument(
        "--sonar-scanner-arch",
        type=str,
    ...
    parser.add_argument(
        "--skip-jre-provisioning", action="store_true", help="If provided, the provisioning of the JRE will be skipped"
    )
    parser.add_argument(
